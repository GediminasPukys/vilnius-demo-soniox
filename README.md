...
this project bundles a local `soniox_tts/` package (ported from the user's
...
| TTS | Soniox `tts-rt-v1` (local `soniox_tts/` plugin), voice "Adrian", `language="lt"` |
...
soniox_tts/         # Local Soniox TTS LiveKit plugin (REST + WebSocket)
  __init__.py
  tts.py
knowledge/
...
- The Soniox TTS plugin is bundled in this repo (`soniox_tts/`). When/if it
  gets adopted into the official `livekit-plugins-soniox` package,
  swap `import soniox_tts` → `from livekit.plugins.soniox import TTS`.
  The local package is named `soniox_tts` (not `soniox`) on purpose —
  importing a top-level `soniox` would shadow the official
  `livekit.plugins.soniox` STT plugin used in the same file.
